module MapGraphHelper
  ...
  def generate_map_all params
    ...
    items = items.select("support_type, count(distinct(users.id)) as count, states.abbreviation as abbreviation")
  ...
  def generate_map_state params
    ...
    items = items.joins("join zips on users.zip_code = zips.zip")
    items = items.joins("join states on zips.state_id = states.id")
    items = items.select("support_type, count(distinct(users.id)) as count, zips.zip, zips.latitude, zips.longitude")
